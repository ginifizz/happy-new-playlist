...
useEffect(() => {
  ...
  return (
    <div ref={container} className='w-full h-screen relative'>
      ...
      <div className='container relative h-full flex flex-row gap-6 items-center mx-auto'>
        <div ref={textElement} className='relative flex-1 flex flex-col gap-1'>
          <div className='text-red'>
            <Arrows />
          </div>
          <p className='text-2xl text-brown font-black'>{subtitle}</p>
          <p className='font-title text-7xl text-red uppercase'>{title}</p>
          <p className='text-lg mt-2'>{description}</p>
          <div className='flex flex-row flex-wrap gap-4 text-beige my-4'>
            <button
              className='flex flex-row gap-2 bg-red py-2 px-3 rounded-3xl transition-all hover:brightness-125'
              onClick={() => {
                switch (playerStatus) {
                  case 'unready':
                    Player.play();
            ...
            >
              ...
              <span className='block uppercase font-black'>
                {playerStatus === 'playing' ? 'Pause' : 'Preview'}
              </span>
            </button>
            <button className='flex flex-row gap-2 bg-green py-2 px-3 rounded-3xl transition-all hover:brightness-125'>
              <svg
                xmlns='http://www.w3.org/2000/svg'
                x='0px'
                y='0px'
              ...
              <span className='uppercase font-black'>Ecouter sur Spotify</span>
            </button>
          </div>
          <button
        ...
        <div ref={tapeElement} className='relative flex-1 h-full'>
          <Tape
            title={title}
            subtitle={subtitle}
            animated={playerStatus === 'playing'}
            className={clsx(
              'absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 drop-shadow-floating w-[90%]',
              rotateClassName
            )}
          />
        </div>
